...
//! `gwae init`, does the picture on my screen change to show me the thing I
...
impl Sandbox {
    ...
    fn config(&self) -> String {
        std::fs::read_to_string(self.dir.join("gwae/gwae.toml")).unwrap_or_default()
    }
}
...
/// A running `gwae init` on a PTY, that keys can be typed into and whose
