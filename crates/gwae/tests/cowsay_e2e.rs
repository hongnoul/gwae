...
/// How many frames have been painted. gwae wraps each frame in the
...
/// screen. Enough for these assertions: gwae only emits CSI, OSC and the
...
fn paint(config_body: &str, cols: u16, rows: u16) -> String {
    ...
    assert!(
        frame_count(&out) >= 1,
        "gwae never painted; captured {} bytes",
        out.len()
    );
