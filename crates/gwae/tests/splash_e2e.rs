...
//! must survive being replayed inside a gwae pane.
//!
...
//! twice:
//!
//! 1. `gwae init` on a bare PTY: does the card actually animate (many
//!    distinct frames), and does it get out of the way of question 1?
//! 2. `gwae run "gwae init"` - the same card drawn *inside* a hosted
//!    pane, which is the real question: a gwae pane is a vt100 grid
//!    recomposited by gwae's own renderer, so animation that relies on
...
fn init_animates_the_title_card_before_the_first_question() {
    let sb = Sandbox::new("");
    // Stop once the flow has reached question 1 (the harness pick, which
    // leads the onboarding since it owns the first choice): that proves
    // both that the card played and that it handed over.
    let out = capture(&sb, &["init"], 80, 24, |s| {
        strip_ansi(s).contains("Agent harness")
    });
    ...
    let first_q = plain.find("Agent harness").unwrap();
...
fn a_keypress_skips_the_card_immediately() {
    ...
    while Instant::now() < deadline {
        // Question 1 is the harness pick now; `z` is not a valid answer
        // there either, so a leak would still be visibly ignored.
        if strip_ansi(&String::from_utf8_lossy(&out)).contains("Agent harness") {
            break;
        }
    ...
    assert!(plain.contains("Agent harness"), "never reached question 1");
    assert!(
...
fn the_card_renders_inside_a_gwae_pane() {
    // The actual question this feature raises: a gwae pane is a hosted
    // vt100 grid recomposited by gwae's own renderer, so the animation has
...
fn narrow_panes_fall_back_to_the_plain_word() {
    ...
    // questions underneath it). The art is 15 cells wide (4 glyphs), so a
    // quarter-width pane of a 48-col terminal (12 cells, ~10 after the inset)
