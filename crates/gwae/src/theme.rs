//! The chrome palette: every color gwae itself paints.
//!
//! Panes are real PTYs that paint themselves, so gwae only themes its own
...
//!    terminal's own ANSI 0-15 colors, so gwae matches whatever the
