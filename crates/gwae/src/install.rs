...
/// gwae setup: onboarding must never be able to install software on a
...
pub const SKIP_ENV: &str = "GWAE_NO_INSTALL";
...
/// `PATH`: the environment was inherited when gwae started, long before
...
mod tests {
    ...
    fn a_just_installed_package_manager_is_still_findable() {
        ...
        assert_eq!(
            resolve("gwae-no-such-program-xyz"),
            std::path::PathBuf::from("gwae-no-such-program-xyz")
        );
    }
