...
//!
//! A keystroke's round trip is `OS -> terminal -> gwae -> pane -> echo
//! back out`, so latency is not gwae's alone to fix: the OS key-repeat
...
//!
//! - **gwae's own config** is ours to write, so `tune` fixes it directly.
...
///
/// `sync_to_monitor no` is normally a tearing tradeoff, but gwae emits
/// synchronized-update markers (`ESC[?2026h/l`) around every frame, so the
/// host applies each repaint atomically anyway. Under gwae you get the
...
pub fn kitty_settings() -> Vec<Setting> {
    ...
    vec![
        ...
        Setting {
            ...
            why: "caps drawing at the monitor refresh; gwae already prevents tearing itself (ESC[?2026)",
...
pub fn audit(input_poll_ms: u64) -> Vec<Setting> {
    ...
    v.extend(gwae_settings(input_poll_ms));
    v
}
...
/// Settings gwae will write itself, versus ones only the user can apply.
...
pub fn ours_and_theirs<'a>(pending: &[&'a Setting]) -> (Vec<&'a Setting>, Vec<&'a Setting>) {
    pending.iter().partition(|s| s.layer == "gwae")
}
...
/// The block telling the user how to apply what gwae will not apply
...
pub fn render_manual_steps(theirs: &[&Setting]) -> Option<String> {
    ...
    if !mac.is_empty() {
        s.push_str(&format!(
            "\n{BOLD}Run once, then log out and back in{RESET}{DIM} (these are machine-wide, so gwae will not set them for you){RESET}\n"
        ));
        for set in &mac {
            if let Some(f) = &set.fix {
                s.push_str(&format!("  {CYAN}{f}{RESET}\n"));
...
mod tests {
    ...
    #[test]
    fn only_gwae_settings_are_ever_applied_automatically() {
        ...
        let (ours, theirs) = ours_and_theirs(&p);
        assert!(
            ours.iter().all(|s| s.layer == "gwae"),
            "only gwae settings are auto-applied"
        );
        assert!(
            theirs.iter().all(|s| s.layer != "gwae"),
            "non-gwae settings must be left to the user"
        );
    ...
    fn kitty_advice_is_withheld_when_not_running_under_kitty() {
        ...
        // it implies gwae does not know what it is talking about.
        if !in_kitty() {
