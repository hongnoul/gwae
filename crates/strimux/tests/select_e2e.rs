...
//! writes to a file, so the assertion is on exactly the bytes strimux handed
...
/// The name of the clipboard helper strimux prefers on this platform. The stub
/// must shadow *that* program, since it is the first one strimux tries and the
...
fn drag_highlights_pane_text_and_copies_it_to_the_clipboard() {
    ...
    assert!(
        String::from_utf8_lossy(&boot).contains("HELLO-STRIMUX"),
