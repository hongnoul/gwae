...
/// Resolve a terminal color to RGB components so it can be blended.
///
/// `Idx` is expanded through the standard xterm-256 cube (the 16 system
/// colors use the widely-shared xterm defaults, since the real ones live in
/// the host terminal and are not knowable from here), and `Default` resolves
/// to `fallback` — for a background that is the theme's `base`, which is what
/// the user actually sees under an unstyled cell.
pub fn to_rgb(c: CColor, fallback: (u8, u8, u8)) -> (u8, u8, u8) {
    match c {
        CColor::Rgb(r, g, b) => (r, g, b),
        CColor::Default => fallback,
        CColor::Idx(i) => {
...
/// Blend `top` over `bottom` with `alpha` (0.0 = fully transparent, 1.0 =
/// fully opaque), resolving both through [`to_rgb`] against `fallback`.
///
/// Terminals have no real alpha channel, so translucency has to be faked by
/// mixing the two colors ourselves and emitting the result as a truecolor
/// cell. That is exactly what a compositor would do, and it is why the
/// blended output is always `Rgb` even when both inputs were indexed.
pub fn blend(top: CColor, bottom: CColor, alpha: f32, fallback: (u8, u8, u8)) -> CColor {
