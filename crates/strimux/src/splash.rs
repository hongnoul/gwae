...
//!
//! Why this exists: `strimux init` used to drop the user straight onto
...
//!   terminal, and `strimux init --print-splash` shows the real thing.
...
//! also have to survive being run *inside* a strimux pane (a hosted vt100
...
/// Deliberately a private table of just these seven letters: a general font
...
/// Every frame, concatenated, for `strimux init --print-splash` and docs.
...
mod tests {
    ...
    fn no_line_ever_exceeds_the_terminal_width() {
        ...
        // strimux pane it would also corrupt the pane below.
        let p = Palette::default();
    ...
    fn a_narrow_terminal_still_names_the_tool() {
        ...
        assert!(s.contains("strimux"), "fallback lost the word: {s:?}");
    }
    ...
    fn frames_only_use_sgr_so_a_hosted_pane_can_replay_them() {
        // The card is also drawn inside a strimux pane. Cursor moves, alt
