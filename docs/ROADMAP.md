...
- [x] Cargo workspace scaffold, CI green on macOS
...
- [x] `$mod` = `⌥+hjkl` (Option key) focus / `⌥+Shift+hjkl` move
...
Themes/truecolor, VHS gifs, docs, Homebrew tap, prebuilt
binaries, `gwae setup`. **Exit**: v1.0.1 installable in one command (`brew install hongnoul/tap/gwae`).
...
installed (config, then receipt, then path) and prints the exact command —
`brew upgrade gwae` for the canonical Homebrew install. A once-a-day background
