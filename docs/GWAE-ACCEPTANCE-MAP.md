...
| A5 | `crates/gwae/tests/pdf_e2e.rs` run against the installed binary via `GWAE_E2E_BIN`, plus the direct `a=q` `o=z` probe answered by the live Ghostty window. Real tdf, real PTY, measured host output volume before and after the change. |
...
| R11 | A PDF pane must not make the whole session feel laggy, since one thread paints every pane. | A5: real tdf under the installed GWAE on a real PTY. Host output at startup fell from 855,885 to 81,053 bytes and a page turn from ~818 KB to ~44 KB once uploads used `o=z`, and every emitted transfer is compressed. A still page costs under 0.2% of a core and 20 page turns under 3%. A live `a=q` query to real Ghostty returns `OK` for an `o=z` transfer, and page turns render correctly on screen. | **PASS** at the wire/redraw boundary. Perceived smoothness is still user-judged, and R09 remains separate. |
...
| O21 | Host upload volume per redraw, and the `o=z` payloads the host must inflate. | A5 `real_tdf_page_redraws_stay_within_a_streamable_frame_budget`: drives real tdf, fails if the image path was never exercised or any transfer is uncompressed, and bounds bytes per page turn. P1 `graphics_enabled_real_child_probe_errors_and_quiet_canonical_upload` and `graphics_two_real_panes_own_same_client_id_and_survive_sibling_deletion` now inflate host payloads and assert they equal the exact declared raster. U1 `canonical_upload_chunks_are_bounded_quiet_and_direct` checks the `o=z` header, round trip and size reduction; `full_pane_page_upload_stays_small_enough_to_stream_per_frame` bounds a full-pane page. **PASS**. |
