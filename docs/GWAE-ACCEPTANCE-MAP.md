...
| R11 | A PDF pane must not make the whole session feel laggy, since one thread paints every pane. | A5: real tdf under the installed GWAE on a real PTY. Host output at startup fell from 855,885 to 81,053 bytes once uploads used `o=z`, and every emitted transfer is compressed. A live `a=q` query to real Ghostty returns `OK` for an `o=z` transfer, and page turns render correctly on screen. | **PASS** at the wire/redraw boundary. Perceived smoothness is still user-judged, and R09 remains separate. |
