...
image_clipboard = false        # true: ⌥+Shift+c copies focused pane as PNG image to clipboard
...
| `image_clipboard` | bool | `false` | When `true`, `⌥+Shift+c` (`Ç`) renders the focused pane's visible grid to a PNG image (embedded 8×8 font, no external deps) and copies `image/png` to the system clipboard (`wl-copy -t image/png` / `xclip -t image/png` / macOS `osascript` `«class PNGf»` / Windows `Set-Clipboard`). When the clipboard helper is unavailable (e.g. over SSH) it writes a temp file and toasts the path instead. Off by default because image copy has no OSC 52 fallback. |
