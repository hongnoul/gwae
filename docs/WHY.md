...
- **vs tmux**: the table's echo row is from gwae's old 1ms poll loop; the loop is event-driven since 1.6.1 and re-measurement puts the two at near-parity (tmux ~0.27 ms vs gwae ~0.35 ms median, same harness, 2026-09-22) — both invisible in practice. gwae wins memory at 4 panes and needs no daemon.
- So **why gwae over tmux, if echo is a wash?** Because the comparison tmux cannot enter is the capability table below: layout. tmux crams N panes into one screen; gwae's panes never shrink.
