...
strimux is **MIT**. Your contributions are MIT too.

## Read-only constraint: niri

niri is **GPL-3.0**. strimux is MIT, so we **cannot read or port niri source**.
...
2. Then the code PR links to it and adds/updates the relevant `proptest`
   invariants in `crates/strimux-layout/tests/invariants.rs`.
